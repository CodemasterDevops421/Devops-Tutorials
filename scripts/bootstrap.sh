...
echo "chaithu ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/99_chaithu
...
# Ensure ssh service is running before reload
...
### 11. LOGWATCH ###
sed -i 's/^MailTo = root/MailTo = root/' /etc/logwatch/conf/logwatch.conf || true
sed -i 's/^Detail = Low/Detail = Medium/' /etc/logwatch/conf/logwatch.conf || true
...
if [[ -n "$AZURE_WORKSPACE_ID" && -n "$AZURE_WORKSPACE_KEY" ]]; then
  ...
  bash onboard_agent.sh -w "$AZURE_WORKSPACE_ID" -s "$AZURE_WORKSPACE_KEY"
  systemctl enable omsagent || true
fi
